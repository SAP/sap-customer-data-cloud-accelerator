import fs from 'fs';
import { srcDirectory, srcFile, templateWebSdkPath, expectedGlobalConf } from './testCommon';
import { initWebSdk } from './initWebSdk';
  ...
  it('should throw an error when reset is false and srcDirectory already exists', async () => {
    ...
    try {
      ...
      expect(error.message).toBe(
        `The "${srcDirectory}" directory already exists, to overwrite it's contents please use the option "reset" instead of "init"`
      );
    }
