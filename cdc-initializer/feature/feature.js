...
import {BUILD_DIRECTORY, FEATURE, FEATURE_NAME_LIST, SRC_DIRECTORY} from "../constants";
...
class Feature {
    ...
    async init(sites, featureName, environment, reset) {
        try {
            ...
            for (const { apiKey, siteDomain = '' } of sites) {
                ...
                await this.#initFeatures(apiKey, siteConfig, siteDomain, reset)
            }
    ...
    async #initFeatures(apiKey, siteConfig, siteDomain, reset) {
    ...
    #buildFeatures(siteDomain) {
        for(const feature of this.#features) {
            const workingDir = path.join(BUILD_DIRECTORY, siteDomain)
